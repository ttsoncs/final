...
void Network::backward(const Matrix &input, const Matrix &target) {
  ...
  for (int i = n_layer - 2; i > 0; i--) {
    layers[i]->backward(layers[i - 1]->output(),
                        layers[i + 1]->back_gradient());
  }
...
void Network::check_gradient(const Matrix &input, const Matrix &target,
  ...
  for (int i = 0; i < n_points; i++) {
    ...
    std::cout << "[layer " << layer_id << ", param " << param_id
              << "] deriv = " << deriv[layer_id][param_id]
              << ", est = " << deriv_est
              << ", diff = " << deriv_est - deriv[layer_id][param_id]
              << std::endl;
...
// References: https://github.com/aammya8/LeNet5_CNN/blob/main/src/network.cc
