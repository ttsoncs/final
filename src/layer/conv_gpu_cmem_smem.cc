#include "./conv_gpu_cmem_smem.h"
#include <iostream>
#include <math.h>

#define TILE_WIDTH 16

void Conv_GPU_CMEM_SMEM::init() {
...
void Conv_GPU_CMEM_SMEM::im2col(const Vector &image, Matrix &data_col) {
  ...
  for (int c = 0; c < channel_in; c++) {
    ...
    for (int i = 0; i < hw_out; i++) {
      ...
      int start_idx = step_h * width_in * stride +
                      step_w * stride; // left-top idx of window
      for (int j = 0; j < hw_kernel; j++) {
        int cur_col = start_idx % width_in + j % width_kernel -
                      pad_w; // col after padding
        int cur_row = start_idx / width_in + j / width_kernel - pad_h;
        if (cur_col < 0 || cur_col >= width_in || cur_row < 0 ||
            cur_row >= height_in) {
          data_col(i, c * hw_kernel + j) = 0;
        } else {
          // int pick_idx = start_idx + (j / width_kernel) * width_in + j %
          // width_kernel;
...
void Conv_GPU_CMEM_SMEM::forward(const Matrix &bottom) {
  ...
  gpuInterfaceCMEM_SMEM.conv_forward_gpu_prolog(
      output, input, weight_, &d_output, &d_input, &d_weight, n_sample,
      channel_out, channel_in, height_in, width_in, height_kernel);

  // Hand off to GPU for computation
  gpuInterfaceCMEM_SMEM.conv_forward_gpu(d_output, d_input, d_weight, n_sample,
                                         channel_out, channel_in, height_in,
                                         width_in, height_kernel);
  cudaDeviceSynchronize();

  // Data transfer GPU to CPU
  gpuInterfaceCMEM_SMEM.conv_forward_gpu_epilog(
      output, d_output, d_input, d_weight, n_sample, channel_out, channel_in,
...
void Conv_GPU_CMEM_SMEM::col2im(const Matrix &data_col, Vector &image) {
  int hw_in = height_in * width_in;
  int hw_kernel = height_kernel * width_kernel;
  int hw_out = height_out * width_out;
  // col2im
    ...
    for (int i = 0; i < hw_out; i++) {
      ...
      for (int j = 0; j < hw_kernel; j++) {
        ...
        if (cur_col < 0 || cur_col >= width_in || cur_row < 0 ||
          ...
          // width_kernel;
          int pick_idx = cur_row * width_in + cur_col;
          image(c * hw_in + pick_idx) +=
              data_col(i, c * hw_kernel + j); // pick which pixel
        }
      }
    }
  }
...
void Conv_GPU_CMEM_SMEM::backward(const Matrix &bottom,
                                  const Matrix &grad_top) {
  int n_sample = bottom.cols();
  grad_weight.setZero();
  grad_bias.setZero();
  ...
  for (int i = 0; i < n_sample; i++) {
    ...
    Matrix grad_top_i_col = Eigen::Map<Matrix>(
        grad_top_i.data(), height_out * width_out, channel_out);
...
void Conv_GPU_CMEM_SMEM::update(Optimizer &opt) {
...
std::vector<float> Conv_GPU_CMEM_SMEM::get_parameters() const {
...
void Conv_GPU_CMEM_SMEM::set_parameters(const std::vector<float> &param) {
...
std::vector<float> Conv_GPU_CMEM_SMEM::get_derivatives() const {
