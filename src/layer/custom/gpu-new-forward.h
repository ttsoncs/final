...
public:
  ...
  void conv_forward_gpu_full(float *host_y, const float *host_x,
                             ...
                             const int C, const int H, const int W,
                             const int K);
  /* For creating a dummy kernel call so that we can distinguish between kernels
   * launched for different layers in the Nsight Compute CLI for measuring per
   * layer Op Times
   */
  void insert_post_barrier_kernel();
  // For inserting a marker visible in Nsys so that we can time total student
  // function time
