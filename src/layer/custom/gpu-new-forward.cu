...
#define k4d(i3, i2, i1, i0)                                                    \
  ...
  int h = (blockIdx.z / W_grid) * TILE_WIDTH +
          threadIdx.y; // row of the image matrix
  int w = (blockIdx.z % W_grid) * TILE_WIDTH +
...
__host__ void GPUInterface::conv_forward_gpu_prolog(
  ...
  int outputSize =
      B * M * H_out * W_out * sizeof(float);    // output feature map is M
...
__host__ void GPUInterface::conv_forward_gpu(
  ...
  // launch the kernel
  conv_forward_kernel<<<numBlocksInGrid, numThreadsPerBlock>>>(
