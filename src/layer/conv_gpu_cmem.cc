#include "./conv_gpu_cmem.h"
#include <iostream>
#include <math.h>

#define TILE_WIDTH 16

void Conv_GPU_CMEM::init() {
...
void Conv_GPU_CMEM::im2col(const Vector &image, Matrix &data_col) {
...
void Conv_GPU_CMEM::forward(const Matrix &bottom) {
  ...
  gpuInterfaceCMEM.conv_forward_gpu_prolog(
  ...
  gpuInterfaceCMEM.conv_forward_gpu(d_output, d_input, d_weight, n_sample,
                                    channel_out, channel_in, height_in,
                                    width_in, height_kernel);
  cudaDeviceSynchronize();

  // Data transfer GPU to CPU
  gpuInterfaceCMEM.conv_forward_gpu_epilog(output, d_output, d_input, d_weight,
...
void Conv_GPU_CMEM::col2im(const Matrix &data_col, Vector &image) {
  int hw_in = height_in * width_in;
  int hw_kernel = height_kernel * width_kernel;
  int hw_out = height_out * width_out;
  // col2im
...
void Conv_GPU_CMEM::backward(const Matrix &bottom, const Matrix &grad_top) {
  int n_sample = bottom.cols();
  grad_weight.setZero();
  grad_bias.setZero();
...
void Conv_GPU_CMEM::update(Optimizer &opt) {
...
std::vector<float> Conv_GPU_CMEM::get_parameters() const {
...
void Conv_GPU_CMEM::set_parameters(const std::vector<float> &param) {
...
std::vector<float> Conv_GPU_CMEM::get_derivatives() const {
