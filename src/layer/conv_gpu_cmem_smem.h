#ifndef SRC_LAYER_CONV_GPU_CMEM_SMEM_H_
#define SRC_LAYER_CONV_GPU_CMEM_SMEM_H_

#include "../layer.h"
#include "./custom/gpu-new-forward-cmem-smem.h"
#include <chrono>
#include <cuda_runtime.h>
#include <vector>

class Conv_GPU_CMEM_SMEM : public Layer {
private:
  const int dim_in;
  int dim_out;
  ...
  GPUInterfaceCMEM_SMEM gpuInterfaceCMEM_SMEM;

  void init();

public:
  Conv_GPU_CMEM_SMEM(int channel_in, int height_in, int width_in,
      ...
      : dim_in(channel_in * height_in * width_in), channel_in(channel_in),
        ...
        stride(stride), pad_w(pad_w), pad_h(pad_h) {
    init();
  }
