...
int main(int argc, char *argv[]) {
  ...
  for (int epoch = 0; epoch < n_epoch; epoch++) {
    std::cout << epoch + 1 << "-th epoch" << std::endl;
    shuffle_data(dataset.train_data, dataset.train_labels);
    for (int start_idx = 0; start_idx < n_train; start_idx += batch_size) {
      int ith_batch = start_idx / batch_size;
      Matrix x_batch = dataset.train_data.block(
          0, start_idx, dim_in, std::min(batch_size, n_train - start_idx));
      Matrix label_batch = dataset.train_labels.block(
          0, start_idx, 1, std::min(batch_size, n_train - start_idx));
  ...
  dnn.save_parameters("../model/weights_cpu.bin");
  return 0;
}
